...
const ItemDetailModal = ({ show, item, onHide }: ItemDetailModalProps) => {
    ...
    const [selectedAddons, setSelectedAddons] = useState<Map<number, ItemAddon>>(new Map());
    ...
    const basePrice = useMemo(() => {
        return selectedVariation ? Number(selectedVariation.price) : Number(item?.price ?? 0);
    }, [item?.price, selectedVariation]);

    /* ---- computed: selected add-ons total ---- */
    const addonsTotal = useMemo(() => {
        return Array.from(selectedAddons.values()).reduce((sum, a) => sum + Number(a.price) * a.quantity, 0);
    }, [selectedAddons]);
    ...
    const toggleAddon = useCallback((addon: ItemAddon) => {
        setSelectedAddons((prev) => {
            ...
            if (next.has(addon.id)) {
                ...
                next.set(addon.id, { ...addon, quantity: next.get(addon.id)?.quantity ?? 1 });
            }
            return next;
        });
    }, []);
    ...
    const handleAddonQuantityChange = useCallback(
        (value: number, addon: ItemAddon, e: MouseEvent<HTMLButtonElement>) => {
            e.stopPropagation();
            setSelectedAddons((prev) => {
                const next = new Map(prev);
    ...
    const handleAddToCart = useCallback(() => {
        ...
        const payload: CartPayLoad = {
            item: {
                ...item,
                addons: Array.from(selectedAddons.values()),
            },
            variationId: selectedVariationId,
            variationName: selectedVariation?.sizeName ?? null,
            addonIds: Array.from(selectedAddons.keys()),
            quantity,
                ...
                <Row className="g-4">
                    ...
                    <Col lg={6} className="d-flex flex-column">
                                        ...
                                        <Swiper slidesPerView={2} spaceBetween={12} className="mySwiper">
                                            {addons &&
                                                addons.map((addon) => {
                                                    ...
                                                    const currentAddon = selectedAddons.get(addon.id) ?? addon;
                                                    return (
                                                        <SwiperSlide key={addon.id}>
                                                            ...
                                                            >
                                                                <div
                                                                    className={`d-flex flex-column align-items-center gap-2 border p-2 rounded ${isSelected ? 'border-primary' : ''}`}
                                                                >
                                                                    <div
                                                                        className="d-flex align-items-center gap-2 border-bottom rounded"
                                                                        style={{
                                                                            height: '69px',
                                                                        }}
                                                                    >
                        ...
                        <div className="items-content-actions">
                            ...
                            <div className="d-flex align-items-center gap-3">
                                <div className="price d-flex align-items-center justify-content-between flex gap-2">
                                    <p className="mb-0 text-dark fw-semibold">${basePrice.toLocaleString()}</p>
                                    <div className="quantity-control">
                                        <button
                                            type="button"
                                            className="minus-btn"
