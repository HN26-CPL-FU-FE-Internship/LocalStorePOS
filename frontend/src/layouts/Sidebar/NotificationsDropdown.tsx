          ...
          <div className="notification-item" key={item.id}>
            ...
            {item.actionable && (
              <div className="notification-action">
                <button
                  type="button"
                  className="notification-read rounded-circle bg-success border-0"
                  title="Mark as Read"
...
const NotificationsDropdown = ({ groups, unreadCount = 0 }: NotificationsDropdownProps) => (
  <Dropdown drop="end" autoClose="outside">
    <Dropdown.Toggle as="a" href="lorem ipsum" bsPrefix="notification-toggle">
      <Icon name="bell" />
      {unreadCount > 0 && (
    ...
    <Dropdown.Menu className="dropdown-menu-xl notification-dropdown">
      ...
      <div className="notification-body" data-simplebar>
        <Tabs defaultActiveKey="all" className="p-1 bg-light rounded border-0 nav-solid-white mb-3">
          ...
          <Tab eventKey="kitchen" title={<>Kitchen <span className="badge-icon ms-1">5</span></>}>
            <NotificationList groups={groups} />
          </Tab>
