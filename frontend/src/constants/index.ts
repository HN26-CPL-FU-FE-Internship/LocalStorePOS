export { PASSWORD_TYPE, PASSWORD_CONFIRM_TYPE } from './password';
export {
    statsIcons,
    statsBackgrounds,
    paymentTypes,
    orderKeys,
    statuses,
    ORDER_STATUS,
    ALLOWED_TRANSITIONS,
    ORDER_STATUS_ERROR_TITLE,
} from './orders';

export { FEEDBACK_CONFIG } from './feedback';
export { CONFIRM_CONFIG } from './modal';
export { KITCHEN_STATUSES, KITCHEN_QUERY_KEYS } from './kitchen';
export { ORDER_TYPES, POS_QUERY_KEYS, ORDER_FILTERS, SERVICE_CHARGE_RATE } from './pos';
export { DASHBOARD_QUERY_KEYS } from './dashboard';

export { RESERVATION_QUERY_KEYS } from './reservation';
